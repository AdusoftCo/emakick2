...
<body>
    ...
    <div class="col-12 mt-4 mb-4 text-center text-pink">
        <h2>SECCION DAMAS</h2>
    </div>
    ...
    $sql = "SELECT id, descripcion, precio_oferta, imagen FROM cami_son_setas WHERE descripcion LIKE '%Camison%'";

    $imagesWithDescriptions = $conexion->consultar($sql);

    if ($imagesWithDescriptions === false) {
        die("Error executing SQL query: " . $conexion->getError());
    }
