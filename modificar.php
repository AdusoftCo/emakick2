...
if ($_GET && isset($_GET['modificar'])) {
    ...
    // Fetch 'Cantidad-Docenas' from Stocking
    //$stockData = $conexion->consultar("SELECT cant_doce FROM stocking WHERE product_id = $id");

    // Store 'Cantidad-Docenas' value in a variable
    $cantidad_docenas = !empty($stockData) ? $stockData[0]['cant_doce'] : 0;    
}
...
if ($_POST) {
    ...
    $cantidad_docenas = $_POST['cantidad'];
    $costo = $_POST['costo'];
    ...
    if ($id_proyecto === false) {
        ...
        $errorInfo = $conexion->pdo->errorInfo();
                ...
                <div class="card" style="background-color: #f2f0f7;">
                    <div class="card-body">
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label for="cantidad" class="form-label">Cantidad-Docenas</label>
                                <input required class="form-control" type="number" name="cantidad" id="cantidad"
                                    value="<?php echo $cantidad_docenas; ?>">
                            </div>
